...
export default function LoginModal({ onDone }) {
  ...
  const { loginWithGoogle, continueAsGuest, error, loading: authLoading } = useAuth()
  ...
  const handleGoogle = async () => {
    ...
    // onDone will be triggered by auth state change in App.jsx
  }
      ...
      <div style={{
        ...
        {mode === 'choose' && (
          <>
            ...
            {/* Error */}
            {error && (
              <div style={{ fontSize:12, color:'#FF6B6B', fontWeight:700, marginTop:10 }}>
                {error}
              </div>
            )}
            ...
            <div style={{ marginTop:20, textAlign:'left', padding:'0 8px' }}>
              ...
              {[
                ...
                { icon:'🏆', text:'Nama & foto di leaderboard global' },
                { icon:'🔒', text:'Tidak ada yang bisa pakai nama kamu' },
              ].map((b, i) => (
                <div key={i} style={{
                  display:'flex', alignItems:'center', gap:8,
                  fontSize:12, color:textMuted, lineHeight:1.8,
                }}>
        ...
        {mode === 'guest' && (
          <>
            <p style={{ fontSize:13, color:textMuted, marginBottom:16, lineHeight:1.6 }}>
              Skor tamu hanya tersimpan di device ini. Kamu bisa login nanti kapan saja.
            </p>
            ...
            <div style={{ position:'relative', marginBottom:8 }}>
              <input
                value={guestName}
                onChange={(e) => { setGuestName(e.target.value.slice(0, 20)); setNameError('') }}
            ...
            <div style={{ display:'flex', gap:8, marginTop:8 }}>
              ...
              <button
                ...
                style={{
                  flex:2, padding:'13px',
                  background: guestName.trim().length >= 2
                    ? 'linear-gradient(135deg,#A29BFE,#6C5CE7)'
                    : (dark ? '#1e2a4a' : '#eee'),
                  color: guestName.trim().length >= 2 ? '#fff' : textMuted,
                  border:'none', borderRadius:14, fontSize:15, fontWeight:800,
                  fontFamily:"'Fredoka One',cursive",
                  cursor: guestName.trim().length >= 2 ? 'pointer' : 'default',
                  boxShadow: guestName.trim().length >= 2 ? '0 6px 20px rgba(162,155,254,0.4)' : 'none',
                  transition:'all 0.2s',
                }}>
                Mulai Main
              </button>
