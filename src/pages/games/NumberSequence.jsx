...
// ─── Component ────────────────────────────────────────────────────────────
export default function NumberSequence({ onBack, game, difficulty }) {
    ...
    return (
      <div style={{ minHeight:'100dvh', background:bg, display:'flex', flexDirection:'column', alignItems:'center', justifyContent:'center', padding:20 }}>
        ...
        <div style={{ textAlign:'center', maxWidth:420, width:'100%' }}>
          ...
          <h1 style={{ fontFamily:"'Fredoka One',cursive", color:won?'#00B894':'#FF6B6B', fontSize:26, margin:'0 0 4px' }}>{won ? 'LUAR BIASA!' : 'GAME OVER'}</h1>
          <p style={{ color:textMuted, fontSize:14, marginBottom:20 }}>{won ? `Kamu master pola angka!` : `Level ${level} — ${LEVEL_NAMES[level]||''}`}</p>
          ...
          <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:10, marginBottom:20, background:surface, borderRadius:16, padding:16 }}>
            {[{ l:'Skor',v:score.toLocaleString(),i:'🎯'},{ l:'Level',v:level,i:'📈'},{ l:'Benar',v:`${totalCorrect}/${totalAnswered}`,i:'✅'},{ l:'Akurasi',v:`${accuracy}%`,i:'🎯'},{ l:'Best Streak',v:bestStreak,i:'🔥'},{ l:'Koin',v:`+${coinReward}`,i:'🪙'}].map((s,i) => (
